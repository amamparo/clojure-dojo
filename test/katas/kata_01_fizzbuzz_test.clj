(ns katas.kata-01-fizzbuzz-test
  (:require [clojure.test :refer [deftest is testing]]
            [matcher-combinators.test]
            [katas.kata-01-fizzbuzz :refer [fizzbuzz]]))

(deftest empty-input
  (is (match? [] (fizzbuzz 0))))

(deftest small-cases
  (is (match? [1] (fizzbuzz 1)))
  (is (match? [1 2] (fizzbuzz 2)))
  (is (match? [1 2 "Fizz"] (fizzbuzz 3)))
  (is (match? [1 2 "Fizz" 4 "Buzz"] (fizzbuzz 5))))

(deftest first-fifteen
  (is (match? [1 2 "Fizz" 4 "Buzz" "Fizz" 7 8 "Fizz" "Buzz"
               11 "Fizz" 13 14 "FizzBuzz"]
              (fizzbuzz 15))))

(deftest length-matches
  (is (match? 100 (count (fizzbuzz 100))))
  (is (match? 1000 (count (fizzbuzz 1000)))))

(deftest divisibility-rules
  (let [out (fizzbuzz 100)
        at #(nth out (dec %))]
    (testing "every multiple of 15 is FizzBuzz"
      (doseq [i (range 15 101 15)]
        (is (match? "FizzBuzz" (at i)) (str "i=" i))))
    (testing "multiples of 3 not 5 are Fizz"
      (doseq [i (range 1 101)
              :when (and (zero? (mod i 3)) (pos? (mod i 5)))]
        (is (match? "Fizz" (at i)) (str "i=" i))))
    (testing "multiples of 5 not 3 are Buzz"
      (doseq [i (range 1 101)
              :when (and (zero? (mod i 5)) (pos? (mod i 3)))]
        (is (match? "Buzz" (at i)) (str "i=" i))))
    (testing "non-multiples of 3 or 5 are integers equal to i"
      (doseq [i (range 1 101)
              :when (and (pos? (mod i 3)) (pos? (mod i 5)))]
        (is (match? i (at i)) (str "i=" i))))))
