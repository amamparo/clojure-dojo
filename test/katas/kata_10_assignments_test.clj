...
(deftest empty-program (is (= {} (interpret []))))

(deftest single-set (is (= '{x 5} (interpret [[:set 'x 5]]))))

(deftest multiple-sets
  (is (= '{x 5, y 3, z 7} (interpret [[:set 'x 5] [:set 'y 3] [:set 'z 7]]))))

(deftest set-can-rebind (is (= '{x 8} (interpret [[:set 'x 5] [:set 'x 8]]))))

(deftest add-uses-prior-bindings
  (is (= '{x 5, y 3, sum 8}
         (interpret [[:set 'x 5] [:set 'y 3] [:add 'x 'y 'sum]]))))

(deftest mul-uses-prior-bindings
  (is (= '{a 2, b 3, c 6}
         (interpret [[:set 'a 2] [:set 'b 3] [:mul 'a 'b 'c]]))))
...
(deftest dest-can-overwrite-a-source
  (testing "the destination symbol may already be bound"
    (is (= '{x 8, y 3} (interpret [[:set 'x 5] [:set 'y 3] [:add 'x 'y 'x]])))))
...
(deftest unbound-second-source-throws
  (is (thrown-match? clojure.lang.ExceptionInfo
                     ...
                     (interpret [[:set 'x 5] [:mul 'x 'missing 'z]]))))
